...
const AdditionalInfo = ({ product }) => {
  const tabsData = [
    { title: 'Mô tả', content: <Description product={product} />, key: 1 },
    { title: 'Đánh giá', content: <Comment reviews={DUMMY_REVIEWS} />, key: 2 },
  ]
      ...
      <TabContext value={value}>
        <Tabs onChange={handleChange} value={value} aria-label="product info tabs">
          {tabsData.map((tab, index) => (
            <Tab label={tab.title} value={index.toString()} key={tab.key} />
          ))}
        </Tabs>
        {tabsData.map((tab, index) => (
